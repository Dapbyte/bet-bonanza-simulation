<div class="max-w-5xl">
    ...
    <div class="space-y-6">
        ...
        <section class="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6">
            <h2 class="mb-1 text-lg font-bold text-white">Per-User Probability</h2>
            <p class="mb-4 text-xs text-zinc-500">Atur peluang kemunculan per user berdasarkan jendela N spin.</p>

            @if ($userSaveMessage)
                <div
                    class="mb-4 rounded-xl border p-3 text-xs font-semibold {{ str_contains($userSaveMessage, 'Error') ? 'border-red-500/30 bg-red-500/20 text-red-400' : 'border-green-500/30 bg-green-500/20 text-green-400' }}">
                    {{ $userSaveMessage }}
                </div>
            @endif
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div class="rounded-lg bg-black/20 p-4">
                    <label class="mb-2 block text-sm text-zinc-400">User</label>
                    <select wire:model.live="selectedUserId"
                        class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm text-white outline-none focus:border-yellow-500/50 focus:ring-2 focus:ring-yellow-500/50">
                ...
                <div class="rounded-lg bg-black/20 p-4">
                    <label class="mb-2 block text-sm text-zinc-400">Spin Window (N)</label>
                    <input type="number" wire:model.live="userSpinWindow" min="1" step="1"
                        inputmode="numeric"
                    ...
                    <div class="mt-1 text-[10px] text-zinc-500">Dipakai: {{ $userWindowSpinCount }} /
                        {{ $userSpinWindow }}</div>
                </div>

                <div class="rounded-lg bg-black/20 p-4">
                    <label class="mb-2 block text-sm text-zinc-400">Scatter Rate (%)</label>
                    <input type="number" wire:model.live="userScatterRate" min="0" max="100" step="1"
                        inputmode="numeric"
                        class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-white outline-none focus:border-yellow-500/50 focus:ring-2 focus:ring-yellow-500/50">
                </div>

                <div class="rounded-lg bg-black/20 p-4">
                    <label class="mb-2 block text-sm text-zinc-400">Bomb Rate (%)</label>
                    <input type="number" wire:model.live="userBombRate" min="0" max="100" step="1"
                        inputmode="numeric"
                        class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-white outline-none focus:border-yellow-500/50 focus:ring-2 focus:ring-yellow-500/50">
                    <div class="mt-1 text-[10px] text-zinc-500">Bomb aktif saat Free Spins.</div>
                </div>
            </div>

            <div class="mt-3 rounded-lg bg-black/20 p-4">
                <label class="mb-2 block text-sm text-zinc-400">Enable Per-User Rates</label>
                <label class="relative inline-flex cursor-pointer items-center">
                    <input type="checkbox" wire:model="userSettingsEnabled" class="peer sr-only">
                    <div
                    ...
                    <span class="ml-3 text-sm text-zinc-300"
                        x-text="$wire.userSettingsEnabled ? 'Enabled' : 'Disabled'"></span>
                </label>
            </div>
                ...
                @foreach ($symbolNames as $key => [$emoji, $name])
                    <div class="flex items-center gap-3 rounded-lg bg-black/20 p-3">
                        ...
                        <div class="min-w-0 flex-1">
                            <label class="block text-xs text-zinc-400">{{ $name }}</label>
                            <div class="mt-1 flex items-center gap-1">
                                <input type="number" wire:model.live="userSymbolRates.{{ $key }}"
            ...
            <div class="mt-4 flex flex-wrap items-center gap-3">
                <span class="text-sm text-zinc-400">Symbol total:</span>
                <span class="text-lg font-black text-yellow-400">{{ $this->userSymbolRatesTotal }}%</span>
                <span class="text-xs text-zinc-500">Scatter + Bomb total:</span>
                <span class="text-lg font-black text-yellow-400">{{ $userScatterRate + $userBombRate }}%</span>
                <span class="text-xs text-zinc-500">Grand total:</span>
                <span class="text-lg font-black text-yellow-400">{{ $this->userTotalRate }}%</span>
            </div>

            <div class="mt-4 flex justify-end">
                <button wire:click="saveUserSettings" wire:loading.attr="disabled"
                    class="rounded-xl bg-gradient-to-r from-yellow-500 via-amber-500 to-yellow-500 px-6 py-2 text-xs font-bold uppercase tracking-wider text-black shadow-lg shadow-yellow-500/25 transition-all hover:from-yellow-400 hover:via-amber-400 hover:to-yellow-400 active:scale-95 disabled:opacity-50">
                    <span wire:loading.remove wire:target="saveUserSettings">Save User Settings</span>
                    <span wire:loading wire:target="saveUserSettings">Saving...</span>
                </button>
            </div>
        </section>
        ...
        <section class="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div class="rounded-lg bg-black/20 p-4">
                    ...
                    <input type="number" wire:model="defaultBalance" min="0" step="1"
                        inputmode="numeric"
                        class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-white outline-none focus:border-yellow-500/50 focus:ring-2 focus:ring-yellow-500/50">
                </div>
                ...
                <div class="rounded-lg bg-black/20 p-4">
                    ...
                    <input type="number" wire:model="maxMultiplier" min="1" step="1"
                        inputmode="numeric"
                        class="w-full rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-white outline-none focus:border-yellow-500/50 focus:ring-2 focus:ring-yellow-500/50">
                </div>
            </div>
